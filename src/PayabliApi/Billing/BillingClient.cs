...
public partial class BillingClient : IBillingClient
{
    ...
    private async Task<WithRawResponse<BillingProfileQueryResponse>> ListProfilesAsyncCore(
        long orgId,
    ...
    {
        var _queryString = new PayabliApi.Core.QueryStringBuilder.Builder(capacity: 6)
            .Add("profileName", request.ProfileName)
            .Add("feeType", request.FeeType)
            .Add("serviceVertical", request.ServiceVertical)
            .Add("profileId", request.ProfileId)
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<BillingProfileQueryResponse>(
                    responseBody
                )!;
                return new WithRawResponse<BillingProfileQueryResponse>()
                {
                    Data = responseData,
    ...
    private async Task<WithRawResponse<BillingProfileResponse>> GetProfileAsyncCore(
    ...
    )
    {
        ...
        var response = await _client
            .SendRequestAsync(
                new JsonRequest
                {
                    Method = HttpMethod.Get,
                    Path = string.Format(
                        "billing/configuration/{0}/{1}/{2}",
                        ValueConvert.ToPathParameterString(serviceGroup),
                        ValueConvert.ToPathParameterString(entityType),
                        ValueConvert.ToPathParameterString(entityId)
                    ),
                    QueryString = _queryString,
                    Headers = _headers,
                    Options = options,
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<BillingProfileResponse>(responseBody)!;
                return new WithRawResponse<BillingProfileResponse>()
                {
                    Data = responseData,
                    RawResponse = new PayabliApi.RawResponse()
                    {
    ...
    /// );
    /// </code></example>
    public WithRawResponseTask<BillingProfileQueryResponse> ListProfilesAsync(
        long orgId,
        ListBillingProfilesRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    ...
    {
        return new WithRawResponseTask<BillingProfileQueryResponse>(
            ListProfilesAsyncCore(orgId, request, options, cancellationToken)
        );
    }
    ...
    /// );
    /// </code></example>
    public WithRawResponseTask<BillingProfileResponse> GetProfileAsync(
        GetProfileBillingRequestServiceGroup serviceGroup,
        GetProfileBillingRequestEntityType entityType,
        long entityId,
        RequestOptions? options = null,
    ...
    {
        return new WithRawResponseTask<BillingProfileResponse>(
            GetProfileAsyncCore(serviceGroup, entityType, entityId, options, cancellationToken)
        );
    }
}
